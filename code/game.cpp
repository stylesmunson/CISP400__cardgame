...
int main()
{
	...
	load_tokens(player1_tokens, player2_tokens);
	...
	while (window.isOpen())
	{
		...
		if (gamestate == GameState::PLAYING_P2)
		{
			...
			if (playstate == PlayState::PLAYER2_MONSTERPHASE)
			{
				...
				gamescreen.display_hand(player2hand, { 84, 175, 190, 170 });					//show hand
		...
		if (gamestate == GameState::PLAYING_P1)
		{
			...
			for (auto i : player1hand)
				window.draw(*i);
			for (auto i : player1_battlezone)
				window.draw(*i);
		}
		...
		if (gamestate == GameState::PLAYING_P2)
		{
			...
			for (auto i : player2hand)
				window.draw(*i);
			for (auto i : player2_battlezone)
				window.draw(*i);
		}
